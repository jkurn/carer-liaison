// Cloudflare Worker — Carer Liaison email API
...
function corsHeaders(origin, env) {
  const allowed = (env.ALLOWED_ORIGIN || 'https://carerliaison.com').split(',').map(s => s.trim());
  const isAllowed = allowed.includes(origin) || origin === 'http://localhost:3000' || origin === 'http://127.0.0.1:5500';
  return {
    ...CORS_HEADERS,
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname !== '/api/send' && url.pathname !== '/') {
    ...
    const { email, name, score, tier, answers } = body;
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#F6F5EF;padding:32px 16px;">
  <tr><td align="center">
    ...
    if (!resendRes.ok) {
      ...
      return json({ error: 'Failed to send email. Please try again.' }, 500, origin, env);
    }
    ...
    const adminCtx = fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${env.RESEND_API_KEY}`,
        'Content-Type': 'application/json',
    ...
    // Ensure admin email sends even after response is returned
    ctx.waitUntil(adminCtx);
